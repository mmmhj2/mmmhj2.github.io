...
数字签名提供比报文鉴别码更强大的保证。
如果能保证私钥仅有发送方所有，那么数字签名提供了**不可否认性**（Non-repudiation），只要一个报文具有数字签名，那么发送方就不能否认这个报文来自自己。
相对地，MAC在生成过程中不使用任何加密或解密过程，因此速度更快，但是其只能确认报文未受修改，不具有不可否认性，因此是*可否认的鉴别*。
...
使用**会话ID**（Session ID）或**会话令牌**（Session token）的鉴别和不重数的核心思想相同。
