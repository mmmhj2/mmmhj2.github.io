...
对任何寻找特征值的问题，都可以通过试探函数和变分原理将其转化为一个优化问题。
物理上的许多问题，从振动的模态到定态下的波函数，都可以转化为特征值问题，从而利用瑞丽-里兹法进行求解，因此这个方法在物理问题的数值计算中具有特别重要的地位。

这一节中我们将介绍一般的特征值问题的解法。
...
那么，特征值问题
$$T_{\mathcal L} u = \lambda u$$
的解，是原问题的解一个近似。
{: .proposition}

集合$\mathcal L$的构造方式多种多样，一般选择正交的多项式函数，这些函数称为试探函数（test function/ansatz function）。

对于希尔伯特空间空间，有一类特殊情况，即泛函构成的希尔伯特空间。
这种空间中，算子$T$一般是一个微分算子。
这是物理中最常见到的一种情况，也就是我们之前介绍的那种情况。
在这种情况下，我们则利用 [Lax-Milgram 定理]({% post_url optimisation/2024-09-23-differential-problems %}#Lax-Milgram定理)，先将问题转化为弱形式，再利用瑞利-里兹法求解。

设$T$为一希尔伯特空间$\mathcal L$（譬如平方可积函数空间）上的微分算子，对问题
$$T\phi = L, \quad \phi, L \in \mathcal L,$$
其对应弱形式的二次型为
$$(T \phi, \psi) \implies A(\phi, \psi),$$
那么，通过构造近似
$$(T_A)_{i,j} = A(\phi_i, \phi_j),$$
得出的问题
$$T_A u = L, \quad u = \sum_{i=1}^n a_i \phi_i$$
的解，是原问题解的一个近似。
数值上讲，这意味着求解线性系统
$$T_A a = F, \quad F = (L\phi_1, \dots, L\phi_n)^\top.$$
{: .proposition}
...
从数学原理上讲，这是因为微分算子$T$作用在$\phi$上后，其像可能不在希尔伯特空间中了，且具有高阶微分的微分算子，很难证明其满足 Lax-Milgram 定理中要求的性质，尤其是对称性。
因此我们需要寻找等价的弱形式。
