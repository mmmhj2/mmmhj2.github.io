...
其中$\psi\_i$是一个*满足边界条件（运动学许可）*的简单函数，称为试探函数（Test function），通常是多项式函数。
...
一般来说，瑞利-里兹法是连接特征值问题和优化问题的桥梁。
这个方法的思路是，首先将无穷维问题或者高维问题降维到有限维或低维，然后将特征值问题转化为优化问题求解。
对许多物理问题，我们都可以将其转化为寻找特征值的问题：从振动的模态到定态下的波函数，都可以转化为特征值问题，从而利用瑞利-里兹法进行求解，因此这个方法在物理问题的数值计算中具有特别重要的地位。

首先我们说明从特征值问题到优化问题的桥梁。

设 $M$ 为一厄米矩阵，$x$ 为一非零向量，定义
$$R(M,x) = \frac{x^\star M x}{x^\star x}$$
为瑞利商（Rayleigh quotient）。
设$\lambda\_\max, \lambda\_\min \in \mathbb R$为$M$特征值中最大和最小者，则
$$\forall x \neq 0, \; \lambda_\text{min} \le R(M,x) \le \lambda_\text{max},$$
且其最大值和最小值必在对应的特征向量处取得。
{: .proposition}

根据谱定理，易知 $M$ 可对角化。
不妨设其特征值从小到大分别为
$$\lambda_\text{min} = \lambda_1 \le \cdots \le \lambda_n = \lambda_\text{max}, $$
则有
$$M = P^\star D P, \; D = \mathrm{diag}(\lambda_1, \dots, \lambda_n).$$
记 $y = Px$，则
$$R(M,x) = \frac{y^\star D y}{y^\star y} = \frac{\sum_{i=1}^n \lambda_i y_i^\star y_i}{\sum_{i=1}^n y_i^\star y_i}.$$
这是$\lambda\_i$的*凸组合*，其最小值因此为$\lambda\_1$，并且在对应坐标为一的时候取得：
$$
\frac{y_1^\star y_1}{\sum_{i=1}^n y_i^\star y_i} = 1 \implies y_1 = 1,\; y_2 = \cdots = y_n = 0.
$$
此时$x$必然处于对应的特征空间中，因此是对应的特征向量。对最大的特征值，证明同理。
{: .proof}

更一般地，以$\lambda\_2$为例，任意特征值可以通过先限制$x$正交于更小的特征向量组成的空间（此时为$\lambda\_1$的特征向量），再求最小值来取得。
根据以上推理，任何求厄米矩阵特征值的问题都可以转化为一个极值问题。

---

然后，我们来讨论如何从无穷维空间降维近似。

设$\mathcal H$为一希尔伯特空间，其内积记为$(\cdot, \cdot)$。
设$T$为其上一个线性算子。
我们希望求解其特征值问题，即找出满足
$$T(u) = \lambda u, \quad u \in \mathcal H, \lambda \in \mathbb C$$
的$\lambda$。
那么，设
$$\mathcal L = \{ \phi_1, \phi_2, \dots, \phi_n \},$$
为一组$\mathcal H$上的函数，这些函数可用来构成$T$的一个有限维的近似$T\_{\mathcal L}$，其矩阵形式为：
$$(T_{\mathcal L})_{i, j} = (T \phi_i, \phi_j).$$
若$\mathcal L$能够张成$\mathcal H$的一个子空间，那么$T_{\mathcal L}$是$T$从$\mathcal H$到$\mathcal L$的一个压缩（Compression），从而广义特征值问题
$$T_{\mathcal L} u = \lambda M u, \; M = (\phi_i, \phi_j)$$
的解，是原问题的解在子空间$\mathcal L$上的近似。
这个矩阵$M$就是质量矩阵。
{: .proposition}

集合$\mathcal L$的构造方式多种多样，一般选择正交的多项式函数，这些函数称为试探函数（test function/ansatz function）。
特别地，如果这些试探函数是正交归一的，那么质量矩阵就是单位矩阵。

---

对于希尔伯特空间，有一类特殊情况，即泛函构成的希尔伯特空间。
这种空间中，算子$T$一般是一个微分算子。
这是物理中最常见到的一种情况，也就是我们之前介绍的那种情况。
在这种情况下，我们则利用 [Lax-Milgram 定理]({% post_url optimisation/2024-09-23-differential-problems %}#Lax-Milgram定理)，先将问题转化为弱形式，再利用类似的方法求解，这种方法也叫里兹-伽辽金（Ritz-Galerkin）法。

设$T$为一希尔伯特空间$\mathcal H$（譬如平方可积函数空间）上的微分算子，对问题
$$T\phi = f, \quad \phi, f \in \mathcal H,$$
其对应弱形式的二次型为
$$(T \phi, \psi) \implies A(\phi, \psi),$$
那么，通过构造近似
$$(T_A)_{i,j} = A(\phi_i, \phi_j),$$
得出的问题
$$T_A u = f, \quad u = \sum_{i=1}^n a_i \phi_i$$
的解，是原问题解的一个近似。
数值上讲，这意味着求解线性系统
$$T_A a = F, \quad F = (f\phi_1, \dots, f\phi_n)^\top.$$
{: .proposition}

一般我们要求这个二次型是正定且是对称的，这样才可以使用上面介绍的方法将特征值问题转化为优化问题。
通常来说，这种二次型一般代表物理系统的能量。

这个弱形式的二次型$A(\phi, \psi)$的构造和估计不是平凡的。
尽管，利用里斯表示定理，我们可以直接使用
$$A(\phi, \psi) = (f \phi, \psi)$$
这一二次型，但这通常是不可取的。
从实践上讲，这是因为微分算子$T$中可能存在高阶微分或难以处理的边界条件，因此难以计算；
从数学原理上讲，这是因为微分算子$T$作用在$\phi$上后，其像可能不在希尔伯特空间中了——这也是引入索伯列夫空间的原因。
另一方面，具有高阶微分的微分算子可能是无界的、甚至没有良好的定义。
一是很难证明其满足 Lax-Milgram 定理中要求的性质，难以验证连续性与强制性；二是很难证明其对称性，从而将其特征值问题转化为优化问题[^non-symmetric]。

[^non-symmetric]: 如果不具有对称性，依然可以使用一般的伽辽金方法，但这不在本文的讨论范围之内。

我们[之前见到过]({% post_url structure-mechanics/2024-03-11-hamiltonian-principle %}#特征模态的表示)，寻找弱形式一般是通过分部积分（或者更一般的格林-斯托克斯公式）完成的。
利用分部积分可将微分次数重新分配，避免二次型中某一项次数过高从而满足对称性且便于计算；
在数学上，则是将其定义域限制在索伯列夫空间上。
...
本节主要介绍求解常系数微分方程组，这是各种数值方法的重要一步。
