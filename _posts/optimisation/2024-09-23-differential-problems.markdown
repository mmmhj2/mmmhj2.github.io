...
\end{aligned}
$$
并存在一连续线性映射$L$，那么存在唯一的$u \in \mathcal H$，使得
...
设$V$为一向量空间，实践上，我们一般需要求解的是下面这样的方程：
$$A(u)= L, \quad u \in V, L \in V'$$
其中$A$是从$V$到其对偶空间$V'$的线性映射。
其等价命题为
$$A(u) (v) = Lv, \quad \forall v \in V.$$
这个命题通常叫做问题的强形式。
我们可以借助里斯表示定理构造二次型：
$$a(u, v) = A(u)(v) = (Au, v),$$
然后将问题变为
$$a(u, v) = Lv,$$
然后利用 Lax-Milgram 定理将其转化为求泛函
$$J(v) = \frac{1}{2} a(v, v) - Lv,$$
最小值的问题，只要这个二次型是对称的即可。
这个被转化后的最小值问题就叫做问题的弱形式（Weak formulation）。
有些时候，弱形式也直接指二次型$a(\cdot, \cdot)$。

很多时候，问题的弱形式并不是直接使用$(Au, v)$那么简单，其中原因之一就是对称性。
如果$A$不满足对称性，那么就无法通过这个定理将其转化为优化问题。
除了对称性之外，我们还需要考虑强制性、边界条件等诸多问题。
有些时候，我们可能需要对$A$进行一些等价变换，最常见的变换就是利用分部积分重新分配微分阶数，这一技巧在结构力学中已经介绍过了。
