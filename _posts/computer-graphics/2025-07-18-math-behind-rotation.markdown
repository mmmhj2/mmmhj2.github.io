...
如果我们希望研究大空间（比如矩阵空间$\mathcal{M}(n)$）中的一个小部分（比如旋转群$SO(n)$）的结构，*流形*（Manifolds）是一种自然的数学工具。
简单来说，流形指的是在任何局部足够平整，以至于能够直线、平面等用线性的空间来近似的几何体[^generality-of-manifolds]。
这种概念非常常见：用直线来近似函数曲线、用平面来研究曲面的局部性质，实际上都是流形的应用。

[^generality-of-manifolds]: 一般来说流形能被定义在任何具有“连续”概念的空间，即拓扑空间上。这里我们只考虑欧几里得空间的子集，在这个范畴下，许多要研究的流形自然具有下文所述微分流形的性质。

数学上，我们要求，对流形$M$中任意一个点的邻域$U$，均存在同胚$\phi\_U$，将其映射到欧几里得空间$\mathbb R^n$的一个开子集上，这个欧几里得空间的维数就是流形的维数，这个同胚则叫做局部坐标卡。
同胚是正逆映射均连续的双射，考虑到欧几里得空间中的微分是良定义的，若这个坐标卡还是微分同胚，那么自然的想法就是利用坐标卡来定义流形上的微分：
对定义在任意点邻域的函数$f: U \to \mathbb R$，按照函数复合的思路，可将其“微分”定义为
$$
\partial f = (f \circ \phi_U^{-1})',
$$
由于这个复合函数是从欧几里得空间到实数的函数，其微分（只要存在）是良定义的。

这个定义虽简单却有一处重大缺陷：如果一个函数定义在两个相交的邻域上，没有任何性质能够保证上文所述的微分在这个邻域的交上是良定义的，因为两个局部坐标卡可能给出不同的映射。
为了修复这一点，我们额外要求，对所有可能相交的邻域$U,V$，映射
$$
\begin{array}{ccccc}
\psi_{U \to V}: & U \cap V & \to & U \cap V \\
& X & \mapsto & (\phi_U \circ \phi_V^{-1})(X)
\end{array},
$$
是微分同胚，这个映射也叫做转移映射（transition map）。
利用转移映射，只要任何函数在一个邻域上可微，那么就必然在这个交集上可微，因此只要要求函数在每个邻域上可微，即可定义出这个函数在流形上的微分。

可逆实矩阵构成的集合$\mathit{GL}(n)$就是上文所说的微分流形的一个特殊的例子。
考虑映射
$$
\begin{array}{ccccc}
\psi: & \mathit{GL}(n) & \to & \mathbb R^{n^2} \\
& \begin{pmatrix} a_{11} & \cdots & a_{1n} \\ \vdots & \ddots & \vdots \\ a_{n1} & \cdots & a_{nn} \end{pmatrix} & \mapsto & (a_{11}, a_{12}, \dots, a_{nn})
\end{array},
$$
也就是将矩阵“拍扁”的映射。
这个映射的像剔除了行列式为零的矩阵，而由于行列式映射是多项式映射，行列式为零的矩阵构成一个闭集合，因此可逆矩阵构成开集合。
显然这个映射限制在像上后是一个微分同胚。
由于这个坐标卡的定义域是全空间，这自然意味着这个空间是微分流形。

### 李群和李代数的定义

为了更仔细地研究旋转矩阵这一微分流形，我们有必要引入进一步的数学工具，即李群和李代数。

*李群*（Lie group）是具有微分流形结构，且群加法和逆元运算可微的群。
{: .definition}

如上文所述，具有微分流形结构实际上是说这个群具有非常良好的两点局部性质：
首先，群上任何一个元素附近都可和一有限维线性空间（欧几里得空间）近似，这就是说李群是一个*流形*；
其次，这个近似足够好，以至于可以用常规的微积分工具来研究该群中的微积分，这就是说李群是一个*微分流形*。

显然，上文提到的可逆矩阵群$GL(n)$就是一个李群——这个群的加法（也就是矩阵乘法）和逆元运算均是多项式映射，因此都是光滑的。
几乎所有具有物理意义的实流形都是李群，包括但不限于正交矩阵群$O(n)$以及我们将要研究的特殊正交群$SO(n)$，即旋转矩阵的群。
...
注意矩阵的转置是线性变换，因此与求导可交换，这一性质也可逐元素验证。
这意味着$S(t)$是一个反对称矩阵。
