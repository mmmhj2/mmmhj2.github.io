...
显然$Q\_j$具有和力相似的功能，因此称为**广义力**（generalized force）。
...
这个方程就称为**第二类拉格朗日方程**（Lagrange's equation of the second kind），简称拉格朗日方程。
{: .theorem}
...
有时也称为正则动量（canonical momentum）。
若该广义坐标是循环坐标，那么其广义动量守恒，即
$$p_j = \frac{\partial L}{\partial \dot q_j} = C_j.$$
{: .proposition}

这是诺特定理（Noether's theorem）的一个特例：空间上的不变性对应了时间上的守恒量。
...
求解方程，可得：
$$\sum_{j=1}^k \frac{\partial L}{\partial \dot q_j} \dot q_j - L = 2 T_2 + T_1 - (T_2 + T_1 + T_0 - V) = H,$$
即
$$T_2 - T_0 + V = H.$$
这表明系统的一部分能量是守恒的，这个能量称为*广义能量*，而这种守恒称为**广义能量守恒**。
对定常约束的系统（此时可选择广义坐标使其不显含时间），$T\_0 = 0$，广义能量守恒就是一般意义下的机械能守恒。

系统的广义能量定义为
$$H = \sum_{j=1}^k \frac{\partial L}{\partial \dot q_j} \dot q_j - L,$$
也称为该系统的*哈密顿量*（Hamiltonian）。
拉氏量不显含时间[^closed-system]的保守系统的广义能量守恒：
$$H = T_2 - T_0 + V.$$
若广义坐标不显含时间（即广义坐标均为自然坐标），那么系统的广义能量就是其机械能。
{: .proposition}

[closed-system]: 可以证明，拉氏量不显含时间，当且仅当该系统是封闭系统。

哈密顿量也可定义为
$$H(\mathbf p, \mathbf q, t) = \sum_{j=1}^k p_j \dot q_j - L(\mathbf q, \mathbf {\dot q}, t),$$
这叫做拉氏量的*勒让德变换*（Legrendre transformation），$(\mathbf p, \mathbf q)$称为广义坐标$\mathbf q$对应的正则坐标（canonical coordinates）。

广义能量中的势能的物理意义是显然的。
考虑到广义速度和速度具有相似的功能，广义能量中的二次项也不难理解。
那么$T\_0$的物理意义是什么呢？
根据$T\_0$的表达式，可以猜测这一项能量是由于广义坐标和自然坐标“不配合”而产生的。
以旋转的系统为例，一般选择转动的角度为广义坐标之一，而非自然的笛卡尔坐标。
此时，如果将离心力（惯性力的一种，参见上文的讨论）视为有势力，那么会产生额外的“势能”。
这种势能在广义能量守恒中以$-T\_0$的形式出现，因此这个势能也叫*离心势能*。
