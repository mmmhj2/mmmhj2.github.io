...
我们之前介绍过对称矩阵的西尔维斯特惯性定理，不过是以双线性形的形式给出的：

<small>（西尔维斯特惯性定理，双线性形版本）</small>设$\varphi$为一对称双线性形式，那么在任何关于$\varphi$正交的基底下，表示$\varphi$的对角矩阵总是有$n^+$个正元素和$n^-$个负元素。
{: .theorem}

将这个定理和谱定理联合起来，可得到惯性定理对对称矩阵的特殊版本。

<small>（西尔维斯特惯性定理，对称矩阵版本）</small>设$A$为一对称矩阵，那么定存在一正交矩阵$P$和一对角矩阵$W$，满足：
$$A = P W D W^\top P^\top,$$
其中
$$W = \mathrm{diag}(\sqrt{\vert P_{1,1}\vert}, \dots, \sqrt{\vert P_{n,n}\vert}),$$
而$D$是对角线上只有$+1, 0, -1$的对角矩阵。
其中正一的个数即为正惯性指数、负一的个数即为负惯性指数。
{: .proposition}
