...
$q(x) > 0$，那么称其为**正定**的，记$q \in \mathcal{Q}^{++}$；
$q(x) \ge 0$，那么称其为**正**的（或**半正定**的），记$q \in \mathcal Q^+$。
同理，也有负定和半负定的二次型。
{: .definition}
...
根据定义，一个显然的推论是，惯性指数只与双线性函数有关，而与其坐标表示，即对称矩阵，所选取的基底无关，无论在该基底下是否能将该双线性函数表示为对角矩阵。
这个结论叫做西尔维斯特惯性定理。

<small>（西尔维斯特惯性定理）</small>在所有关于$\varphi$正交的基底下，表示$\varphi$的对角矩阵恰好有$n^+$个正元素和$n^-$个负元素。
{: .theorem}
