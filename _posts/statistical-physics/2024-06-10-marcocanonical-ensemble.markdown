...
为证明该函数是热力学势，可采用和正则系综一样的思路，即考虑系综和巨正则库组成的孤立系统，然后使用该孤立系统的熵来证明系综的自发演化方向。
...
除了上述三种系综之外，化学中进行分析时常使用的另一种系综。
化学反应一般在等温等压的条件下进行，因此保持温度$T$、压力$p$和物质的量$N$不变的系综，称为*等温等压系综*（isothermal-isobaric ensemble），在化学中特别重要。
这种系综的热力学势是*吉布斯自由能*（Gibbs free energy）：
$$
G = E - TS + pV = F + pV = H - TS,
$$
其中$F$是亥姆霍次自由能、$H$是焓。
这个热力学函数的变化被用来判定等温等压条件下化学反应能否自发进行，即
$$\Delta G = \Delta H - T \Delta S,$$
这是高中化学反应原理中学习过的内容。
这种自发进行的判断是“瞬时”的，因此物质的量可视为不变。
