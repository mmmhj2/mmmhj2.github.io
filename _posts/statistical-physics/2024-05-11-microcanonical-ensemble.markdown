...
这个球面的面积为
$$S = \frac{2 \pi^{3N/2}}{\Gamma(3N/2)} \left(\frac{2mE}{\hbar^2} \right)^{\frac{3N-1}{2}},$$
其中$\Gamma$是伽马函数。
现在我们需要计算单个状态的大小。
观察$K\_x$，注意到每一维的单个状态占据的空间为$2\pi/L$，因此整个系统的单状态大小为
$$S_\text{state} = \left(\frac{2\pi}{L}\right)^{3N} = \frac{(2\pi)^{3N}}{V^n}.$$
两者相除[^simplicity]，再除以由于粒子完全相同而重复计数的状态数$n!$（参见[麦克斯韦-玻尔兹曼近似]({% post_url statistical-physics/2024-06-10-marcocanonical-ensemble %}#麦克斯韦-玻尔兹曼近似)），即得总状态数：
$$\Omega(E, N, V) = \frac{V^N}{N! \cdot \Gamma(3N/2)} \frac{2 \pi^{3N/2}}{(2\pi)^{3N} \hbar^{3N-1}} (2mE)^{\frac{3N-1}{2}}.$$
现在，由于$n$趋于无穷大，我们可以使用几个近似：
$$\Omega(E, n, V) \sim \frac{V^n}{n! \cdot \Gamma(3N/2)} \frac{2 \pi^{3N/2}}{h^{3N}} (2mE)^{\frac{3N}{2}},$$
然后取对数并利用斯特灵近似：
$$\lim_{x \to \infty} \ln \Gamma(x) = x \ln x - x + O(1),$$
得到
$$
\begin{aligned}
\ln \Omega &\simeq N \ln V + \frac{3N}{2} \ln \frac{2 \pi m E}{h^2} - N \ln N - \frac{3N}{2} \ln \frac{3N}{2} + \frac{5N}{2} \\
&= N \ln \frac{V}{N} + \frac{3N}{2} \ln \frac{4 \pi m E}{3N h^2} + \frac{5N}{2} \\
&= N \ln \frac{V}{N} \left( \frac{4 \pi m E}{3N h^2} \right)^\frac{3N}{2} + \frac{5}{2}N.
\end{aligned}
$$
最后得到
...
[^simplicity]: 这里我们直接用“面积”除以“体积”得到状态数，计算更快但并不严谨。正确的做法是：计算能量小于等于$E$的状态数，然后计算能量小于等于$E + \mathrm d E$的状态数，然后两者相减得到“球壳”中的状态数，最后除以单个状态的体积。多余的$\mathrm d E$在最后才会被舍去。
